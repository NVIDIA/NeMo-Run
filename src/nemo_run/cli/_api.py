...
from nemo_run.cli import cache
...
def entrypoint(
    ...
    *,
    ...
    def wrapper(f: F) -> F:
        ...
        if POPULATE_CACHE:
            cache.cache_entrypoint_metadata(f, _entrypoint)
...
def factory(
    ...
    *,
    ...
    def wrapper(fn: Callable[Params, T]) -> Callable[Params, T]:
        ...
        registration = _register_factory(
            as_factory,
            target=target,
        ...
        if POPULATE_CACHE:
            cache.cache_factory_metadata(as_factory, registration)
...
def create_cli(
    ...
    if is_no_cache:
        ...
        if os.path.exists(cache.CACHE_FILE):
            try:
                ...
                CONSOLE.print(f"Cache file removed: {cache.CACHE_FILE}")
            except Exception as e:
                CONSOLE.print(f"Error removing cache file: {e}")
        
        # Remove the flag from sys.argv to avoid confusing other argument parsers
        sys.argv = [arg for arg in sys.argv if arg != "--no-cache"]
    
    app: Typer = Typer(add_completion=not is_lazy, pretty_exceptions_enable=False)
    if is_lazy:
    ...
    # Try to use cache for help requests
    if is_help_request and not is_no_cache:
        try:
            cache_data = cache.load_cache()
            if cache.validate_cache(cache_data):
                CONSOLE.print("[Cached version] This can be disabled with --no-cache", style=rich_utils.STYLE_USAGE)
                return cache.reconstruct_help_from_cache(cache_data, sys.argv)
        except Exception as e:
