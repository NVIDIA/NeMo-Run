...
def some_function(model: TestModel, learning_rate: float = 0.01):
    pass


class TestFromCli:
    def test_from_cli_with_invalid_args(self):
        with pytest.raises(Exception):
            run.Config.from_cli(some_function, "invalid_arg=value")

    @pytest.mark.parametrize("cls", [run.Config, run.Partial])
    def test_from_cli_with_factory(self, cls):
        @run.cli.factory
        def custom_model(hidden=500) -> cls[TestModel]:
            return cls(TestModel, hidden=hidden, activation="gelu")

        result = cls.from_cli(some_function, "model=custom_model", "learning_rate=0.005", "model.hidden=1000")
        assert isinstance(result, cls)
        assert result.model.hidden == 1000
        assert result.model.activation == "gelu"
        assert result.learning_rate == 0.005

        result = cls.from_cli(some_function, "model=custom_model(hidden=1000)")
        assert result.model.hidden == 1000

        result = cls.from_cli(some_function, "model=custom_model(50)")
        assert result.model.hidden == 50
