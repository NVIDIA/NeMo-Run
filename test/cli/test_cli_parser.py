...
class TestOperations:
    def test_addition(self):
        def func(a: int, b: List[int], c: Dict[str, int]): pass
...
class TestExceptions:
    ...
    def test_invalid_key(self):
        def func(a: int): pass
        with pytest.raises(ArgumentValueError, match="Invalid argument: No parameter named 'b' exists for"):
    ...
    def test_invalid_literal(self):
        def func(a: Literal['red', 'green', 'blue']): pass
        with pytest.raises(LiteralParseError, match="Invalid value for Literal type. Expected one of \\('red', 'green', 'blue'\\), got 'yellow'"):
            parse_cli_args(func, ["a='yellow'"])
...
class TestParseValue:
    def test_parse_int(self):
        ...
        with pytest.raises(ParseError, match="Failed to parse '3.14' as <class 'int'>: Invalid integer literal"):
            parse_value("3.14", int)
        with pytest.raises(ParseError, match="Failed to parse 'not_an_int' as <class 'int'>: Invalid integer literal"):
    ...
    def test_parse_float(self):
        ...
        assert parse_value("-inf", float) == float('-inf')
        with pytest.raises(ParseError, match="Failed to parse 'not_a_float' as <class 'float'>: Could not convert string to float"):
    ...
    def test_parse_bool(self):
        ...
        with pytest.raises(ParseError, match="Failed to parse 'not_a_bool' as <class 'bool'>: Cannot convert .* to bool"):
            parse_value("not_a_bool", bool)
        with pytest.raises(ParseError, match="Failed to parse '2' as <class 'bool'>: Cannot convert .* to bool"):
            parse_value("2", bool)
    ...
    def test_parse_union(self):
        ...
        with pytest.raises(ParseError, match="Failed to parse 'true' as typing.Union\\[int, float\\]: No matching type in Union."):
    ...
    def test_parse_optional(self):
        ...
        with pytest.raises(ParseError, match="Failed to parse 'not_an_int' as typing.Optional\\[int\\]: No matching type in Union. Errors: Failed to parse 'not_an_int' as <class 'int'>: Invalid integer literal"):
    ...
    def test_parse_literal(self):
        ...
        with pytest.raises(LiteralParseError, match="Failed to parse 'yellow' as typing.Literal: Invalid value for Literal type. Expected one of \\('red', 'green', 'blue'\\), got 'yellow'"):
            parse_value("yellow", Color)

    def test_parse_nested_types(self):
        assert parse_value("[[1, 2], [3, 4]]", List[List[int]]) == [[1, 2], [3, 4]]
        assert parse_value('{"a": [1, 2], "b": [3, 4]}', Dict[str, List[int]]) == {"a": [1, 2], "b": [3, 4]}
        with pytest.raises(ListParseError, match="Failed to parse '\\[1, 2, 3\\]' as typing.List: Invalid list: Failed to parse '1' as typing.List: Invalid list: Not a list"):
            parse_value("[1, 2, 3]", List[List[int]])

    def test_parse_unknown_type(self):
        class UnknownType:
            pass
        with pytest.raises(UnknownTypeError, match="Failed to parse 'value' as <class '.*UnknownType'>: Unsupported type"):
    ...
    def test_custom_parser(self):
        ...
        with pytest.raises(ParseError, match="Failed to parse 'not_a_complex' as <class 'complex'>: Invalid complex number"):
    ...
    def test_strict_mode(self):
        ...
        with pytest.raises(ParseError, match="Failed to parse 'value' as <class '.*CustomType'>: Unsupported type"):
...
class TestPythonicParser:
    ...
    def test_parse_lambda(self, parser):
        ...
        with pytest.raises(ArgumentValueError):
            parser.parse_lambda("lambda x: globals()['__builtins__']['eval']('__import__(\"os\").system(\"echo hacked\")')")
...
class TestEdgeCases:
    ...
    def test_complex_nested_structures(self):
        def func(a: List[Dict[str, Union[int, List[str]]]]): pass
