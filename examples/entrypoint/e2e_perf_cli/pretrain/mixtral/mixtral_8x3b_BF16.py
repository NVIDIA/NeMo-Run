...
def model() -> run.Config[pl.LightningModule]:
    model_obj = run.Config(MixtralModel, config=run.Config(MixtralConfig8x3B))
...
@run.cli.factory
def mixtral_trainer(num_nodes=2, callbacks=None) -> run.Config[nl.Trainer]:
    callbacks=[
...
def train_model(
        ctx: run.cli.RunContext = run.cli.RunContext(name="pretrain_mixtral_8x3b_bf16_ctx"),
        ...
        log=set_log_dir(
            ...
            exp_name="mixtral_8x3b"
            ),
        resume=llm.default_resume(),
        detach=True,
        track=False,
        exp_name="pretrain_mixtral_8x3b_bf16",
    ):
