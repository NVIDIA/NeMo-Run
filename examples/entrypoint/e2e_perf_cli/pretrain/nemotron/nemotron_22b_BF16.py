...
from nemo.collections.llm.gpt.model.nemotron import Nemotron4Config22B, NemotronModel, NemotronConfig
...
def model() -> run.Config[pl.LightningModule]:
    model_obj = run.Config(NemotronModel, config=run.Config(Nemotron4Config22B))
...
@run.cli.factory
def nemotron_trainer(num_nodes=2, callbacks=None) -> run.Config[nl.Trainer]:
    callbacks=[
...
def train_model(
        ctx: run.cli.RunContext = run.cli.RunContext(name="pretrain_nemotron_15b_bf16_ctx"),
        ...
        log=set_log_dir(
            ...
            exp_name="pretrain_nemotron_22b_bf16"
            ),
