...
from nemo.collections.llm.gpt.model.nemotron import Nemotron4Config340B, NemotronModel, NemotronConfig
...
def model() -> run.Config[pl.LightningModule]:
    model_obj = run.Config(NemotronModel, config=run.Config(Nemotron4Config340B))
...
@run.cli.factory
def nemotron_trainer(num_nodes=16, callbacks=None) -> run.Config[nl.Trainer]:
    callbacks=[
...
@run.cli.factory
def nemotron_distributed_fused_adam_cosine_annealing() -> run.Config[OptimizerModule]:
    opt_cfg = run.Config(
        OptimizerConfig,
        optimizer="adam",
        lr=0.0001,
...
def train_model(
        ctx: run.cli.RunContext = run.cli.RunContext(name="pretrain_nemotron_340b_bf16_ctx"),
        ...
        log=set_log_dir(
            ...
            exp_name="pretrain_nemotron_340b_bf16"
            ),
