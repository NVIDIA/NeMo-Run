...
def model() -> run.Config[pl.LightningModule]:
    model_obj = run.Config(LlamaModel, config=run.Config(Llama3Config8B))
...
@run.cli.factory
def llama3_trainer() -> run.Config[nl.Trainer]:
    callbacks=[
...
@run.cli.factory
def set_log_dir(job_dir, exp_name) -> run.Config[nl.NeMoLogger]:
...
def train_model(
        ctx: run.cli.RunContext = run.cli.RunContext(name="pretrain_llama3_8b_bf16_ctx"),
        ...
        log: nl.NeMoLogger = set_log_dir(
            ...
            exp_name="pretrain_llama3_8b_bf16"
            ),
