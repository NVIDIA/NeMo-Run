...
from nemo.collections.llm.gpt.model.llama import Llama3Config8B, Llama3Config70B, LlamaModel, Llama3Config
...
def eos_executor(
    ...
    _executor = get_base_executor(
        ...
        custom_mounts=[
            ...
            "/lustre/fsw/coreai_dlalgo_ci/datasets/llama3_tokenizer:/lustre/fsw/coreai_dlalgo_ci/datasets/llama3_tokenizer",
            "/lustre/fsw/coreai_dlalgo_ci/datasets/llama/llama_pile:/lustre/fsw/coreai_dlalgo_ci/datasets/llama/llama_pile"
        ],
        custom_env_vars={
            "HF_HOME": "/lustre/fsw/coreai_dlalgo_ci/datasets/llama3_tokenizer",
        },
        use_ft=ft,
        retries=retries,
        time=time,
...
def eos_paths(
    base_path: str = "/lustre/fsw/coreai_dlalgo_nemofw/jjennings/data/decompress_test/",
) -> list[str]:
    paths = []

    for i in range(1):
        paths.append(os.path.join(base_path, f"my-llama_{i:02d}_text_document"))
    for path_ in paths:
...
def model() -> run.Config[pl.LightningModule]:
    model_obj = run.Config(LlamaModel, config=run.Config(Llama3Config70B))
...
@run.cli.factory
def llama3_trainer(num_nodes=8, callbacks=None) -> run.Config[nl.Trainer]:
    callbacks=[
...
@run.cli.factory
def llama_distributed_fused_adam_cosine_annealing() -> run.Config[OptimizerModule]:
    opt_cfg = run.Config(
        OptimizerConfig,
        optimizer="adam",
        lr=0.0001,
...
def train_data(
    ...
    if not index_mapping_dir:
        # index_mapping_dir = DEFAULT_INDEX_MAPPING_DIRS[cluster]
        index_mapping_dir = None

    paths = eos_paths(base_path=base_data_path)
    ...
    return run.Config(
        ...
        tokenizer=run.Config(AutoTokenizer, pretrained_model_name="meta-llama/Meta-Llama-3-70B", use_fast=True),
        split="99990,8,2",
        num_workers=2,
...
def train_model(
        ctx: run.cli.RunContext = run.cli.RunContext(name="pretrain_llama3_70b_bf16_ctx"),
        _executor = eos_executor(),
        model_cfg: pl.LightningModule = model(),
        trainer: nl.Trainer = llama3_trainer(),
        ...
        log: nl.NeMoLogger = set_log_dir(
            ...
            exp_name="pretrain_llama3_70b_bf16"
            ),
